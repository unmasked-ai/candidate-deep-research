...
class ScoreCard(BaseModel):
    ...
    justification: str = Field(
        description="Concise explanation of the score in 100 words or less"
    )
    reasons: List[str] = []
...
class MatchEvaluationInput(BaseModel):
    ...
    company_profile: Optional[dict] = Field(
        default=None, description="Company culture and context"
    )
    company_details: Optional[List[dict]] = Field(
        default=None,
        description="List of company details (alternative to company_profile)",
    )
...
def _generate_justification(
    ...
    if top_reasons:
        reason_text = "; ".join(
            top_reasons[:2]
        )  # Max 2 reasons to stay under word limit
...
def calculate_match_score(match_data: dict) -> str:
    ...
    try:
        ...
        if match_data.get("company_profile"):
            ...
            match_data.get("company_details")
            and isinstance(match_data["company_details"], list)
            and len(match_data["company_details"]) > 0
        ):
        ...
        experience_score = _calculate_experience_score(
            job_spec, candidate, evidence, reasons, missing_data
        )

        # Culture scoring (20% or 10% if no culture data)
        culture_score, culture_weight = _calculate_culture_score(
            job_spec, candidate, company, evidence, reasons, missing_data
        )
        ...
        # Logistics scoring (5%)
        logistics_score = _calculate_logistics_score(
            job_spec, candidate, evidence, reasons, missing_data
        )
...
def _calculate_skills_score(
    ...
    nice_to_have_coverage = (
        len(nice_to_have_matches) / max(len(nice_to_have), 1) if nice_to_have else 0
    )

    # Tech stack alignment
    tech_matches = set(job_tech_stack) & set(candidate_skills)
    tech_coverage = (
        len(tech_matches) / max(len(job_tech_stack), 1) if job_tech_stack else 0
    )

    # Calculate weighted score
    skills_score = int(
        must_have_coverage * 70 + nice_to_have_coverage * 15 + tech_coverage * 15
    )
    ...
    if must_have_matches:
        evidence.append(
            Evidence(
                source="candidate",
                quote=f"Skills: {', '.join(sorted(must_have_matches))}",
                field="skills",
            )
        )
        reasons.append(
            f"matches {len(must_have_matches)}/{len(must_have)} must-have skills"
        )

    if len(must_have) > len(must_have_matches):
        missing_skills = set(must_have) - must_have_matches
        reasons.append(
            f"missing {len(missing_skills)} must-have skills: {', '.join(sorted(missing_skills))}"
        )
...
def _calculate_experience_score(
    ...
    if (
        job_spec.experience_requirements
        and job_spec.experience_requirements.years_min is not None
    ):
        ...
        if candidate_years >= years_min:
            ...
            evidence.append(
                Evidence(
                    source="candidate",
                    quote=f"{candidate_years} years experience",
                    field="experience",
                )
            )
        else:
    ...
    if job_spec.seniority and job_spec.seniority != "unspecified":
        ...
        if job_seniority in seniority_keywords:
            if any(
                keyword in candidate_title
                for keyword in seniority_keywords[job_seniority]
            ):
                score += 20
...
def _calculate_culture_score(
    ...
    if company and company.culture_fit:
        ...
        evidence.append(
            Evidence(
                source="company", quote=f"Culture fit score: {score}", field="culture"
            )
        )
    ...
    if company and company.culture_values:
        ...
        evidence.append(
            Evidence(
                source="company",
                quote=f"Values: {', '.join(company.culture_values[:3])}",
                field="culture",
            )
        )
...
def _calculate_domain_score(
    ...
    if job_spec.industry and candidate.industry_experience:
        ...
        if job_industry in candidate_industries:
            ...
            evidence.append(
                Evidence(
                    source="candidate",
                    quote=f"Industry: {job_spec.industry}",
                    field="domain",
                )
            )
        else:
            ...
            related_matches = any(
                job_industry in ind or ind in job_industry
    ...
    if job_spec.domain_knowledge and candidate.industry_experience:
        domain_overlap = set([d.lower() for d in job_spec.domain_knowledge]) & set(
            [i.lower() for i in candidate.industry_experience]
        )
        if domain_overlap:
            score += 10
...
def _calculate_logistics_score(
    job_spec: JobSpec,
    candidate: CandidateProfile,
    evidence: List[Evidence],
    reasons: List[str],
    ...
    if job_spec.salary_range and candidate.salary_expectation:
        if (
            job_spec.salary_range.max is not None
            and candidate.salary_expectation.min is not None
        ):
...
- job_spec: JobSpec (from role-requirements-builder) - may be in JSON or XML format
- candidate_profile: CandidateProfile (from person-research) - in XML format
- company_profile: CompanyProfile (from company-research) - in XML format (optional)

Goal:
1) Parse inputs (handle both JSON and XML formats for candidate_profile and company_profile).
2) Validate inputs; if missing or malformed, return JSON with error field and validation_failed code.
3) Normalize strings and lists; dedupe; canonicalize tech names.
...
5) Compose overall_score using weights: skills 45%, experience 20%, culture 20% (cap 10% without explicit signal), domain 10%, logistics 5%.
6) Build concise reasons (max 8 short items), missing_data, evidence, and a justification (100 words or less explaining the score).
7) Call the local tool **calculate_match_score** to get the final ScoreCard JSON.
8) Send the ScoreCard JSON back to the interface agent (the ORIGINAL sender) in the SAME thread.

Important rules:
- Parse XML candidate_profile and company_profile inputs by extracting text content from tags
