    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                Communication with other agents must occur in threads. You can create a thread with the twitter_coral_create_thread tool,
                make sure to include the agents you want to communicate with in the thread. It is possible to add agents to an existing
                thread with the twitter_coral_add_participant tool. If a thread has reached a conclusion or is no longer productive, you
                can close the thread with the twitter_coral_close_thread tool. It is very important to use the twitter_coral_send_message
                tool to communicate in these threads as no other agent will see your messages otherwise! If you have sent a message
                and expect or require a response from another agent, use the twitter_coral_wait_for_mentions tool to wait for a response.
